...
function SCHEMA:PlayerDeath(client)
	local char = client:getChar()
		local cl = client:getChar()
	local invitems = cl:getInv():getItems(true)
	...
	if (char) then
		...
		for _, item in pairs(cl:getInv():getItems()) do 	
			
			local itemID = item:getID()
			local it = nut.item.instances[itemID]

			if (item.isWeapon) then
				item:setData("equip", false)	
			end
			
			nut.item.spawn(item.uniqueID, client:GetPos(), nil, client:GetAngles(), item.data)
			cl:getInv():remove(item.id)
		end
